...
int bst::datum_kao_broj(const datum d) {
	return d.dan + d.mjesec * 100 + d.godina * 10000;
}

void bst::insert(const ELTYPE& element, POSITION current) {
	if (datum_kao_broj(element) < datum_kao_broj(current->element)) {
...
bool bst::exists(const ELTYPE& element, POSITION parent) {
	...
	if (datum_kao_broj(element) < datum_kao_broj(parent->element)) {
		return exists(element, parent->left_child);
	}
	else if (datum_kao_broj(element) > datum_kao_broj(parent->element)) {
