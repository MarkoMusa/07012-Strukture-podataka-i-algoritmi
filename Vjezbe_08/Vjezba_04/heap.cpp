...
void heap::reheapify_upward() {
	...
	while (pos > 0 && _elements[pos].sifra[0] < _elements[calc_parent(pos)].sifra[0]) {
...
void heap::reheapify_downward() {
	...
	while (pos < _next / 2) {
		...
		if (left_child >= _next && right_child >= _next) {
			...
			if (_elements[left_child].sifra[0] <= _elements[right_child].sifra[0]) {
		...
		if (_elements[pos].sifra[0] > _elements[index_with_greater_value].sifra[0]) {
